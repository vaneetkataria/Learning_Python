...
cur.execute("SELECT c1, c2, c3 , c4  FROM newtable")
rows = cur.fetchall()
...
try:
 for row in rows:
     ...
     c1 , c2 , c3 , c4 = row
     cur.execute("INSERT INTO newtable1(c0,\
     c1,\
     c2,\
     c3,\
     c4) \
     ...
     #way2--------------
     #stmt = "insert into newtable1(c1, c2, c3, c4) values ({} ,'"+"{}"+"', {}, {})";
     #stmt = stmt.format(row[0], row[1], row[2], row[3])
     #print(stmt)
     #cur.execute(stmt)
     #------------------
