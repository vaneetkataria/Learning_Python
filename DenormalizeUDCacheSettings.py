...
query = "select a.chain as chain, \
         ...
         and a.config_level = b.config_level \
         and a.affiliates = b.affiliates \
         and a.properties = b.properties limit 10"
...
try:
 ...
 try:
     print('Going to insert {} rows'.format(len(rows)))
     for row in rows:
      print('Going to insert row {}'.format(row))
      chain,config_level,affiliates,properties,cache_keys,noncache_keys,cache_neg_rates,force_cache_only,no_override_sourceonly,min_lead_days,max_lead_days,cache_stale_time_value,cache_stale_time_units=row
      cur.execute(
         "INSERT INTO dadt:web.ud_cache_settings(exec_wk ,\
          chain,\
          config_level, \
          affiliates, \
          properties, \
